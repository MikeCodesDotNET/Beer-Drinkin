...
namespace BeerDrinkin.Service.Helpers
{
    ...
    {
        /*
        public static async Task<string> SaveBinaryToStorage(ApiServices services, string blobId, string blobData)
        {
            ...
            var avatarUri = string.Format("{0}{1}/{2}", blobEndpoint.ToString(), ContainerName, blobId.ToLower());

            // Upload the new image as a BLOB from the string.
            CloudBlockBlob blobFromSASCredential =
                container.GetBlockBlobReference(blobId.ToLower());
            await blobFromSASCredential.UploadTextAsync(blobData);
