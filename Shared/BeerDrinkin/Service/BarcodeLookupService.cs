...
namespace BeerDrinkin.Core.Services
{
     ...
     * 
     *  We first want to check if we've already assoicated a beer in our database with a UPC. If we have then we can return the Beer which BeerDrinkin can display with
     ...
     *  selected. This of course could lead to incorrect entries. Its for this reason that we can add multiple barcodes to a Beer and we'll try and deduce incorrect 
     *  products on the backend. 
     * 
     *  Because we will now have a link between a Beer and the RateBeer beer ID, we save this for future use. It means that if we ever get a developer API key for 
